...
const shape = {
    ...
    five:[[12,0],[13,0],[13,1],[14,1]],
    fiveR1:[[12,1],[13,1],[12,2],[13,0]],

    six:[[12,0],[12,1],[12,2],[13,2]],
    sixR1:[[12,0],[13,0],[12,1],[14,0]],
    sixR2:[[12,0],[13,0],[13,2],[13,1]],
    sixR3:[[12,1],[13,1],[14,1],[14,0]]
...
function rotate(arr){
    switch (shapeStatus[0]){
        ...
        case 4:
            if(shapeStatus[1] == 0 ){
                if(arr[2][1] < 27 && panel[arr[2][0]][arr[2][1]+1] != 1){
                    arr.forEach(function(a, b){
            ...
            case 5:
                if(shapeStatus[1] == 0 ){
                    if(arr[2][1] < 27 && panel[arr[2][0]][arr[2][1]+1] != 1){
                        arr.forEach(function(a, b){
                            a[0] = a[0] - shape.five[b][0] + shape.fiveR1[b][0];
                            a[1] = a[1] - shape.five[b][1] + shape.fiveR1[b][1];
                        });
                        shapeStatus[1] = 1;
                    }
                }else if(shapeStatus[1] == 1){
                    if(arr[0][0] > 0 && arr[3][0] < 27 &&  panel[arr[1][0]+1][arr[1][1]] != 1 ){
                        arr.forEach(function(a, b){
                            a[0] = a[0] - shape.fiveR1[b][0] + shape.five[b][0];
                            a[1] = a[1] - shape.fiveR1[b][1] + shape.five[b][1];
                        });
                        shapeStatus[1] = 0;
                    }
                }
                ...
                case 6:
                    if(shapeStatus[1] == 0 ){
                        //最下边的格子快到底时就不能改了 不然会超出界面
                        if(arr[3][0] < 27 && panel[arr[1][0]+2][arr[1][1]] != 1){
                            arr.forEach(function(a, b){
                                a[0] = a[0] - shape.six[b][0] + shape.sixR1[b][0];
                                a[1] = a[1] - shape.six[b][1] + shape.sixR1[b][1];
                            });
                            shapeStatus[1] = 1;
                        }
                    }else if(shapeStatus[1] == 1){
                        //控制变换时可能会超出左右界限
                        if(arr[0][0] >= 1 && arr[3][0] <= 27 && arr[2][1] < 27 && panel[arr[1][0]][arr[1][1]+2] != 1){
                            arr.forEach(function(a, b){
                                a[0] = a[0] - shape.sixR1[b][0] + shape.sixR2[b][0];
                                a[1] = a[1] - shape.sixR1[b][1] + shape.sixR2[b][1];
                            });
                            shapeStatus[1] = 2;
                        }
                    }else if(shapeStatus[1] == 2){
                        if(arr[2][0] < 27 && panel[arr[2][0]+1][arr[2][1]] != 1){
                            arr.forEach(function(a, b){
                                a[0] = a[0] - shape.sixR2[b][0] + shape.sixR3[b][0];
                                a[1] = a[1] - shape.sixR2[b][1] + shape.sixR3[b][1];
                            });
                            shapeStatus[1] = 3;
                        }
                    }else if(shapeStatus[1] == 3){
                         //控制变换时可能会超出左右界限
                         if(arr[0][0] > 0 && arr[3][0] < 27 && panel[arr[3][0]+1][arr[3][1]] != 1 && arr[1][1] < 27){
                            arr.forEach(function(a, b){
                                a[0] = a[0] - shape.sixR3[b][0] + shape.six[b][0];
                                a[1] = a[1] - shape.sixR3[b][1] + shape.six[b][1];
                            });
                            shapeStatus[1] = 0;
                        }
                    }
...
function GameOver(Inv){
    window.confirm("Game over! your score is " + TotalCount);
    clearInterval(Inv);
    ctx.clearRect(0,0,560,560);
    for( var i = 0 ; i < can.clientHeight ; i = i + 20 ){
        for( var j = 0 ; j < can.clientWidth ; j = j + 20){
    ...
    var Inv = setInterval(function(){ 
        ...
        if( shapeArr[2][1] >= border.bottom || !b_drop ){
            ...
            switch(r){
                ...
                case 5:shapeArr = ArrayCopy(shape.five, shapeArr);shapeStatus[0] = 5; shapeStatus[1] = 0;break;
                case 6:shapeArr = ArrayCopy(shape.six, shapeArr);shapeStatus[0] = 6; shapeStatus[1] = 0;break;
            }
        }
        }
        if(!end){
